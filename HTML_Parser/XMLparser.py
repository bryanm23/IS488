...
import re
# These are things to help parse XML documents

def find_xml_files(directory):
    # This finds all XML files in the specified directory and its subdirectories.
    xml_files = []
...
def extract_cpu_and_ram_usage(file_path):
    # This extracts CPU usage, RAM usage, and calculates the mean CPU usage from an XML file using the imported extensions.
    cpu_data = []
    ram_data_percent = []
    ram_data_mb = []
    total_cpu = 0
    ...
    # Iterate over all items and look for the SysHealthCpuComponent and SysHealthMemComponent, which define the metrics we are looking for.
        ...
        if component is not None and component.text == "SysHealthCpuComponent":
            ...
            if utilization is not None and utilization.text:
                try:
                    ...
                    pass  # Ignore invalid values that do not apply or that were inputted incorrectly.
        ...
        if component is not None and component.text == "SysHealthMemComponent":
            ...
            if utilization is not None and utilization.text:
                try:
                    ...
                    # Retrieves the available memory in MB
                    ...
                    pass  # Ignore invalid values that do not apply.
...
def calculate_variance(data, mean):
    ...
    if len(data) > 1:
        ...
        return 0  # Variance is zero if there is only one data point (this is not applicable since all folders are together, but may be useful later.
...
def compare_reports(file_paths):
    ...
    for file_path in file_paths:
        ...
        # Add data to the lists for the variance calculation
        cpu_usage_list.extend(cpu_data)
        ram_usage_percent_list.extend(ram_data_percent)
        
        # Print once per file in the directory
    ...
    # Calculate the variance of CPU and RAM usage. (This is kinda finnicky, maybe?)
    ...
    # Write results to a text file if any increases are found. This can be improved as its pretty weak and just lists the increase. Could have more info.
